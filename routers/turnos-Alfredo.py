...
@router.get("/{turno_id}/detalle", response_model=TurnoOut)
def obtener_detalle_turno(turno_id: int, db: Session = Depends(get_db), current_user: dict = Depends(get_current_user)):
...
def actualizar_turno(turno_id: int, turno_in: TurnoUpdate, db: Session = Depends(get_db), current_user: dict = Depends(get_current_user)):
    ...
    if turno_in.observaciones is not None:
        turno.observaciones = turno_in.observaciones.strip() if (turno_in.observaciones and turno_in.observaciones.strip()) else None
